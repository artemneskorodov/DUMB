...
## 15.05.2026 TASK
### Цель
Внедрить в компилятор поддержку аннотаций **@require** (предусловие) и **@ensure** (постусловие) для функций. Фича должна работать в двух режимах:
- При флаге **--build-debug** проверки включаются в runtime компилируемой программы.
- Без флага **--build-debug** проверок нет. В будующем возможно добавление статических проверок на этапе компиляции.

У аннотация должна быть возможность добавить кастомные строки ошибок. Для этого нужно использовать уже готовую возможность добавлять строки для функций.
Аннотация должна повлиять по большей части на фронтенд компилятора.

### Правила
- Аннотации **@require** и **@ensure** должны размещаться перед функцией в любом количестве.
- Первым аргументом является любое сравнение корректное для языка.
- Вторым аргументом является строка, которая выводится в случае ошибки.
- Внутри аннотации **@ensure** может использоваться неявная переменная **@result**, которая в момент проверки будет установлена в return value функции.
- Проверки **@require** выполняются в начале работы функции до любых других вычислений.
- Проверки **@ensure** выполняются перед возвратом из функции.

### Пример синтаксиса
```
function divide_positive(x, y)
@require(x > 0, "Function is designed to divide only positive numbers")
@require(y > 0, "Function is designed to divide only positive numbers")
